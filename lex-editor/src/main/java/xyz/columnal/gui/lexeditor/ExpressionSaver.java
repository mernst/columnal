...
public class ExpressionSaver extends SaverBase<Expression, ExpressionSaver, Op, Keyword, BracketContent>
{
    public class BracketContent implements StyledShowable
    {
        ...
        public BracketContent(ImmutableList<Expression> expressions, ImmutableList<Pair<Op, CanonicalSpan>> commas)
        {
    ...
    @Override
    protected BracketAndNodes<Expression, ExpressionSaver, BracketContent, Expression> expectSingle(ExpressionSaver this, EditorLocationAndErrorRecorder locationRecorder, CanonicalSpan location)
    {
        return new BracketAndNodes<>(new ApplyBrackets<BracketContent, Expression, Expression>()
        {
            @Override
            public Expression apply(BracketContent items)
            {
            ...
            @Override
            public Expression applySingle(Expression singleItem)
            {
                return singleItem;
            }
    ...
    private ApplyBrackets<BracketContent, Expression, Expression> makeList(ExpressionSaver this, EditorLocationAndErrorRecorder locationRecorder, CanonicalSpan location)
    {
        return new ApplyBrackets<BracketContent, Expression, Expression>()
        {
            ...
            public Expression applySingle(Expression singleItem)
            {
                return locationRecorder.record(location, new ArrayExpression(ImmutableList.<Expression>of(singleItem)));
            }
        };
    }
    ...
    {
        Supplier<ImmutableList<Expression>> prefixKeyword = () -> ImmutableList.<Expression>of(this.<Expression>record(errorDisplayer, new InvalidIdentExpression(keyword.getContent())));
        ...
        {
            Supplier<ImmutableList<Expression>> invalidPrefix = prefixKeyword;
            ...
            ArrayList<Either<Expression, OpAndNode>> precedingItems = currentScopes.peek().items;
            ...
            {
                Expression callTarget = precedingItems.remove(precedingItems.size() - 1).<Expression>either(e -> e, op -> null);
                ...
                if (callTarget != null)
                {
                    ...
                    {
                        @Override
                        public Expression apply(BracketContent args)
                        {
                        ...
                        public Expression applySingle(Expression singleItem)
                        {
                            return apply(new BracketContent(ImmutableList.<Expression>of(singleItem), ImmutableList.of()));
                        }
                    };
                    invalidPrefix = () -> {
            ...
            currentScopes.push(new Scope(errorDisplayer, expect(ImmutableList.of(Keyword.CLOSE_ROUND), close -> new BracketAndNodes<>(applyBracketsFinal.apply(close), CanonicalSpan.fromTo(errorDisplayer, close), ImmutableList.of()), (bracketed, bracketEnd) -> {
                return Either.<Expression, Terminator>left(bracketed);
        ...
        else if (keyword == Keyword.OPEN_SQUARE)
        {
            currentScopes.push(new Scope(errorDisplayer,
                expect(ImmutableList.of(Keyword.CLOSE_SQUARE),
                    ...
                    (e, c) -> Either.<Expression, Terminator>left(e), prefixKeyword, null, true)));
        }
            ...
            currentScopes.push(new Scope(errorDisplayer, expect(ImmutableList.of(Keyword.THEN, Keyword.ELSE, Keyword.ENDIF), miscBracketsFrom(errorDisplayer), (condition, conditionEnd) -> {
                ...
                return Either.right(expect(ImmutableList.of(Keyword.ELSE, Keyword.ENDIF), miscBracketsFrom(conditionEnd), (thenPart, thenEnd) -> {
                    ...
                    return Either.right(expect(ImmutableList.of(Keyword.ENDIF), miscBracketsFrom(thenEnd), (elsePart, elseEnd) -> {
                        ...
                        return Either.<Expression, Terminator>left(locationRecorder.<Expression>record(CanonicalSpan.fromTo(errorDisplayer, elseEnd), ifThenElseExpression));
        ...
        {            
            currentScopes.push(new Scope(errorDisplayer, expectOneOf(errorDisplayer, ImmutableList.of(new Case(errorDisplayer)), Stream.<Supplier<Expression>>of(() -> record(errorDisplayer, new InvalidIdentExpression(keyword.getContent()))))));
        }
        ...
        else
        {
            ...
            while (!consumed)
            {
                ...
                {
                    @Override
                    public <R extends StyledShowable> R fetchContent(BracketAndNodes<Expression, ExpressionSaver, BracketContent, R> brackets)
                    {
    ...
    private ApplyBrackets<BracketContent, Expression, Expression> tupleBracket(ExpressionSaver this, EditorLocationAndErrorRecorder locationRecorder, CanonicalSpan location)
    {
        return new ApplyBrackets<BracketContent, Expression, Expression>()
        {
            @Override
            public Expression apply(BracketContent items)
            {
                ...
                {
                    boolean allOk = true;
                    ArrayList<Pair<String, Expression>> pairs = new ArrayList<>();
                    for (Expression expression : items.expressions)
                    {
                        Pair<String, Expression> p = ((KeyValueExpression)expression).extractPair();
                        if (p == null)
                        {
                            allOk = false;
                            break;
                ...
                else
                {
                    ...
                    {
                        Expression expression = items.expressions.get(i);
    ...
    @Override
    protected <R extends StyledShowable> R makeExpression(List<Either<Expression, OpAndNode>> content, BracketAndNodes<Expression, ExpressionSaver, BracketContent, R> brackets, int innerContentLocation, String terminatorDescription)
    {
        if (content.isEmpty())
        {
            R bracketedEmpty = brackets.applyBrackets.apply(new BracketContent(ImmutableList.of(), ImmutableList.of()));
        ...
        if (collectedItems.isValid())
        {
            ...
            ArrayList<Expression> beforePrevCommas = new ArrayList<>();
            ArrayList<OpAndNode> prevCommas = new ArrayList<>();
            ArrayList<Expression> sinceLastCommaOperands = new ArrayList<>();
            ...
            for (int i = 0; i < validOperands.size(); i++)
            {
                ...
                if (i < validOperators.size() && validOperators.get(i).op == Op.COMMA)
                {
                    ...
                    Expression made = makeExpressionWithOperators(OPERATORS, locationRecorder, (ImmutableList<Either<OpAndNode, Expression>> es) -> makeInvalidOp(location, es), ImmutableList.copyOf(sinceLastCommaOperands), ImmutableList.copyOf(sinceLastCommaOperators), unbracketed);
                    if (made != null)
            ...
            if (e == null)
            {
                ...
                {
                    R invalidOpExpression = brackets.applyBrackets.applySingle(collectedItems.makeInvalid(location, InvalidOperatorExpression::new));
        ...
        if (e == null)
        {
            Expression invalid = collectedItems.makeInvalid(location, InvalidOperatorExpression::new);
    ...
    private BracketAndNodes<Expression, ExpressionSaver, BracketContent, Expression> unbracketed(List<Expression> operands)
    {
        return new BracketAndNodes<>(new ApplyBrackets<BracketContent, Expression, Expression>()
        {
            @Override
    ...
    @Override
    protected Expression makeInvalidOp(CanonicalSpan location, ImmutableList<Either<OpAndNode, Expression>> items)
    {
        InvalidOperatorExpression invalidOperatorExpression = new InvalidOperatorExpression(Utility.<Either<OpAndNode, Expression>, Expression>mapListI(items, x -> x.<Expression>either(op -> locationRecorder.record(op.sourceNode, new InvalidIdentExpression(op.op.getContent())), y -> y)));
    ...
    private Expression keywordToInvalid(Keyword keyword, CanonicalSpan location)
    {
    ...
    private abstract class Choice
    {
        ...
        public abstract Either<Expression, Terminator> foundKeyword(Expression expressionBefore, CanonicalSpan node, Stream<Supplier<Expression>> prefixIfInvalid);
    }
    ...
    private class Case extends Choice
    {
        ...
        private final Pair<CanonicalSpan, Pair<Expression, ImmutableList<Pattern>>> matchAndPatterns;
        ...
        public Case(CanonicalSpan matchKeyword, CanonicalSpan caseLocation, Expression matchFrom, ImmutableList<MatchClause> previousClauses, ImmutableList<Pattern> patternsForCur)
        {
            super(Keyword.CASE);
            this.matchKeyword = matchKeyword;
        ...
        @Override
        public Either<Expression, Terminator> foundKeyword(Expression expressionBefore, CanonicalSpan node, Stream<Supplier<Expression>> prefixIfInvalid)
        {
            final Expression m;
    ...
    private class Given extends Choice
    {
        ...
        public Given(CanonicalSpan matchKeyword, Expression matchFrom, ImmutableList<MatchClause> previousClauses, ImmutableList<Pattern> previousCases, CanonicalSpan caseLocation)
        {
            super(Keyword.GIVEN);
            this.matchKeyword = matchKeyword;
    ...
    private class OrCase extends Choice
    {
        ...
        private final ImmutableList<Pattern> previousCases;
        private final CanonicalSpan caseLocation;
        private final Expression curMatch; // if null, nothing so far, if non-null we are a guard

        private OrCase(CanonicalSpan matchKeyword, Expression matchFrom, ImmutableList<MatchClause> previousClauses, ImmutableList<Pattern> previousCases, CanonicalSpan caseLocation, Expression curMatch)
        {
    ...
    private class Then extends Choice
    {
        ...
        private Then(CanonicalSpan matchKeywordNode, Expression matchFrom, ImmutableList<MatchClause> previousClauses, ImmutableList<Pattern> previousPatterns, CanonicalSpan caseLocation, Keyword precedingKeyword)
        {
            super(Keyword.THEN);
        ...
        public Either<Expression, Terminator> foundKeyword(Expression expressionBefore, CanonicalSpan node, Stream<Supplier<Expression>> prefixIfInvalid)
        {
            ...
            return Either.right(expectOneOf(node, ImmutableList.of(
                ...
                new Choice(Keyword.ENDMATCH) {
                    @Override
                    public Either<Expression, Terminator> foundKeyword(Expression lastExpression, CanonicalSpan node, Stream<Supplier<Expression>> prefixIfInvalid)
                    {
                        ...
                        return Either.<Expression, Terminator>left(locationRecorder.<Expression>record(CanonicalSpan.fromTo(matchKeywordNode, node), matchExpression));
                    }
                }
            ), prefixIfInvalid));
        }
    ...
    public Terminator expectOneOf(CanonicalSpan start, ImmutableList<Choice> choices, Stream<Supplier<Expression>> prefixIfInvalid)
    {
        return new Terminator(choices.stream().map(c -> c.keyword.getContent()).collect(Collectors.joining(" or ")))
        {
        @Override
        public boolean terminate(FetchContent<Expression, ExpressionSaver, BracketContent> makeContent, Keyword terminator, CanonicalSpan keywordErrorDisplayer)
        {
            ...
            for (Choice choice : choices)
            {
                ...
                {
                    // All is well:
                    Expression expressionBefore = makeContent.fetchContent(brackets);
                    Either<Expression, Terminator> result = choice.foundKeyword(expressionBefore, keywordErrorDisplayer, Stream.<Supplier<Expression>>concat(prefixIfInvalid, Stream.<Supplier<Expression>>of(() -> expressionBefore, () -> record(keywordErrorDisplayer, new InvalidIdentExpression(choice.keyword.getContent())))));
            ...
            items.addAll(prefixIfInvalid.<Expression>map(s -> s.get()).collect(Collectors.<Expression>toList()));
            ...
            InvalidOperatorExpression invalid = locationRecorder.<InvalidOperatorExpression>record(CanonicalSpan.fromTo(start, keywordErrorDisplayer), new InvalidOperatorExpression(items.build()));
    ...
    public void saveOperand(Expression singleItem, CanonicalSpan location)
    {
        ...
        if (singleItem instanceof UnitLiteralExpression && curItems.size() >= 1)
        {
            ...
            NumericLiteral num = recent.<NumericLiteral>either(e -> e instanceof NumericLiteral ? (NumericLiteral)e : null, o -> null);
    ...
    @Override
    protected Supplier<Expression> canBeUnary(OpAndNode operator, Expression followingOperand)
    {
        ...
        ImmutableList.<OperatorExpressionInfo>of(
            new OperatorExpressionInfo(Op.COMMA, (lhs, _n, rhs) -> /* Dummy, see below: */ lhs)
            {
                ...
                public OperatorSection makeOperatorSection(EditorLocationAndErrorRecorder locationRecorder, int operatorSetPrecedence, OpAndNode initialOperator, int initialIndex)
                {
                    ...
                    {
                        @Override
                        public <R extends StyledShowable> R makeNary(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> ops, BracketAndNodes<Expression, ExpressionSaver, BracketContent, R> brackets, EditorLocationAndErrorRecorder locationRecorder)
                        {
    ...
    private static Expression makeAddSubtract(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> ops)
    {
    ...
    private static Expression makeTimes(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> _ops)
    {
        return new TimesExpression(args);
    }

    private static Expression makeStringConcat(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> _ops)
    {
        return new StringConcatExpression(args);
    }

    private static Expression makeEqual(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> ops)
    {
        return new EqualExpression(args, ops.get(ops.size() - 1).getFirst() == Op.EQUALS_PATTERN);
    }

    private static Expression makeComparisonLess(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> ops)
    {
    ...
    private static Expression makeComparisonGreater(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> ops)
    {
    ...
    private static Expression makeAnd(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> _ops)
    {
        return new AndExpression(args);
    }

    private static Expression makeOr(ImmutableList<Expression> args, List<Pair<Op, CanonicalSpan>> _ops)
    {
    ...
    @Override
    protected CanonicalSpan recorderFor(Expression expression)
    {
        return locationRecorder.recorderFor(expression);
    }

    @Override
    protected ImmutableList<TextQuickFix> fixesForAdjacentOperands(Expression first, Expression second)
    {
    ...
    public ImmutableMap<DisplayType, Pair<StyledString, ImmutableList<TextQuickFix>>> getDisplayFor(int canonIndex, Node toRightOf)
    {
    ...
    private static class KeyValueExpression extends Expression
    {
        ...
        public Pair<String, Expression> extractPair()
        {
            String lhsIdent = lhs.visit(new ExpressionVisitorFlat<String>()
            {
                @Override
        ...
        public Expression opAsExpression(EditorLocationAndErrorRecorder locationRecorder)
        {
    ...
    private class FunctionParamThen extends Terminator
    {
        ...
        public boolean terminate(FetchContent<Expression, ExpressionSaver, BracketContent> makeContent, Keyword terminator, CanonicalSpan keywordErrorDisplayer)
        {
            final ArrayList<Expression> itemsIfInvalid = new ArrayList<>();
            ...
            {
                @Override
                public BracketContent apply(BracketContent items)
                {
                    return items;
                }

                @Override
                public BracketContent applySingle(Expression singleItem)
                {
            ...
            ArrayList<Expression> params = foundThen ? new ArrayList<>() : null;
            for (int i = 0; i < bracketContent.expressions.size(); i++)
            {
                Expression e = bracketContent.expressions.get(i);
            ...
            if (params != null)
            {
                ...
                currentScopes.push(new Scope(keywordErrorDisplayer, expect(ImmutableList.of(Keyword.ENDFUNCTION), s -> expectSingle(locationRecorder, s), (e, s) -> Either.<Expression, Terminator>left(locationRecorder.<Expression>record(new CanonicalSpan(initialFunction.start, s.end), new LambdaExpression(args, e))), () -> ImmutableList.copyOf(itemsIfInvalid), null, false)));
            }
            else
            {
                // Invalid
                currentScopes.peek().items.add(Either.<Expression, OpAndNode>left(locationRecorder.<Expression>record(new CanonicalSpan(initialFunction.start, itemsIfInvalid.isEmpty() ? initialFunction.end : locationRecorder.recorderFor(itemsIfInvalid.get(itemsIfInvalid.size() - 1)).end), new InvalidOperatorExpression(Utility.<Expression>prependToList(keywordToInvalid(Keyword.FUNCTION, initialFunction), ImmutableList.<Expression>copyOf(itemsIfInvalid))))));
            }
            return foundThen;
        }
    }
    ...
    private class DefineThen extends Terminator
    {
        ...
        public boolean terminate(FetchContent<Expression, ExpressionSaver, BracketContent> makeContent, Keyword terminator, CanonicalSpan keywordErrorDisplayer)
        {
            ...
            ArrayList<DefineItem> definitions = foundThen ? new ArrayList<>() : null;
            for (int i = 0; i < bracketContent.expressions.size(); i++)
            {
                Expression e = bracketContent.expressions.get(i);
                itemsIfInvalid.add(e);
            ...
            if (definitions != null)
            {
                ...
                currentScopes.push(new Scope(keywordErrorDisplayer, expect(ImmutableList.of(Keyword.ENDDEFINE), s -> expectSingle(locationRecorder, s), (e, s) -> {
                    ...
                    return Either.<Expression, Terminator>left(locationRecorder.<Expression>record(new CanonicalSpan(initialDefine.start, s.end), defineExpression));
                }, () -> ImmutableList.copyOf(itemsIfInvalid), null, false)));
            }
            else
            {
                // Invalid
                currentScopes.peek().items.add(Either.<Expression, OpAndNode>left(locationRecorder.<Expression>record(new CanonicalSpan(initialDefine.start, itemsIfInvalid.isEmpty() ? initialDefine.end : locationRecorder.recorderFor(itemsIfInvalid.get(itemsIfInvalid.size() - 1)).end), new InvalidOperatorExpression(ImmutableList.<Expression>copyOf(itemsIfInvalid)))));
            }
            return foundThen;
        }

        // Returns null or the original array reference passed to constructor
        private class VisitDefinitions extends ExpressionVisitorFlat<ArrayList<DefineItem>>
        {
            ...
            @Override
            protected ArrayList<DefineItem> makeDef(Expression expression)
            {
                return null;
            }

            @Override
            public ArrayList<DefineItem> hasType(HasTypeExpression self, String lhsVar, Expression rhsType)
            {
            ...
            @Override
            public ArrayList<DefineItem> equal(EqualExpression self, ImmutableList<Expression> expressions, boolean lastIsPattern)
            {
